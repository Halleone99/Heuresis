...
import { listCardTypes, type AccentKey, type CardType, type PackWithType } from "./heuresis";
...
export async function updateCollection(collectionId: string, patch: {
  title?: string;
  description?: string | null;
  accent?: AccentKey;
  glyph?: string | null;
  ...
  const next = {
    ...
    glyph: patch.glyph === undefined ? undefined : patch.glyph?.trim() || null,
  };
...
export async function createPack(input: {
  ...
  const { data, error } = await db().from("heuresis_packs").insert({
...
export async function updatePack(packId: string, patch: {
...
export async function restorePack(packId: string) {
  const { error } = await db().from("heuresis_packs").update({ archived_at: null }).eq("id", packId);
  if (error) throw error;
}

export async function deletePack(packId: string) {
  const { error } = await db().from("heuresis_packs").delete().eq("id", packId);
  if (error) throw error;
}
...
export async function listCatalogues(): Promise<SavedCatalogue[]> {
  const { data, error } = await db()
    .from("heuresis_catalogues")
    .select("id,title,description,criteria,sort_order,created_at,updated_at")
    .order("sort_order")
    .order("created_at");
  if (error) throw error;
  return (data ?? []).map((row) => ({
    ...
    criteria: catalogueCriteria(row.criteria),
    sort_order: Number(row.sort_order ?? 0),
...
export async function createCatalogue(input: { title: string; description?: string; criteria: CatalogueCriteria }) {
  ...
  if (lastError) throw lastError;
  const { data, error } = await db().from("heuresis_catalogues").insert({
    title: input.title.trim(),
    description: input.description?.trim() || null,
    criteria: input.criteria,
    sort_order: Number(last?.[0]?.sort_order ?? -1) + 1,
  }).select("id,title,description,criteria,sort_order,created_at,updated_at").single();
  if (error) throw error;
  return {
    id: data.id,
    title: data.title,
    description: data.description ?? null,
    criteria: catalogueCriteria(data.criteria),
    sort_order: Number(data.sort_order ?? 0),
    created_at: data.created_at,
    updated_at: data.updated_at,
  } satisfies SavedCatalogue;
}

export async function deleteCatalogue(id: string) {
  const { error } = await db().from("heuresis_catalogues").delete().eq("id", id);
  if (error) throw error;
}

export async function importCards(packId: string, rows: Array<Record<string, string>>) {
  let created = 0;
...
export async function finishSession(sessionId: string) {
  const { error } = await db().from("heuresis_sessions").update({ ended_at: new Date().toISOString() }).eq("id", sessionId);
  if (error) throw error;
}
...
export function typesById(types: CardType[]) {
