...
import { createTopicFromRelatedWords, listRelatedCatalogue, promoteRelatedCard, relationLabel, RELATION_TYPES, removeRelatedRelation, type RelatedCatalogueRow, type RelationType } from "../lib/related";
...
type EditingEntry = { id: string; relationId: string; term: string; reading: string; meaning: string };
type EntryStatus = "raw" | "flashcard";
...
function statusOf(row: RelatedCatalogueRow): EntryStatus {
  return row.target_role === "main" ? "flashcard" : "raw";
}

function uniqueEntryCount(rows: RelatedCatalogueRow[]) {
...
export default function RelatedCatalogueView({ packs, collections, collection = null, onBack, onOpenPack, onTopicCreated }: Props) {
  ...
  const [statusFilters, setStatusFilters] = useState<EntryStatus[]>([]);
  ...
  async function reload() {
    ...
    catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Could not load connections."); }
  ...
  const selectedRawCardIds = useMemo(() => Array.from(new Set(selectedRows.filter((row) => row.target_role === "related").map((row) => row.target_card_id))), [selectedRows]);
  const uniqueEntries = uniqueEntryCount(shown);
  ...
  const availableRelationTypes = useMemo(() => RELATION_TYPES.filter((value) => collectionRows.some((row) => row.relation_type === value)), [collectionRows]);
  ...
  async function saveEdit() {
    ...
    try {
      ...
      setError(editError instanceof Error ? editError.message : "Could not update the entry.");
  ...
  async function promote(row: RelatedCatalogueRow) {
    ...
    catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Could not make this entry a flashcard."); }
  }

  async function promoteSelected() {
    if (!selectedRawCardIds.length || bulkBusy) return;
    setBulkBusy(true); setError("");
    try {
      await Promise.all(selectedRawCardIds.map((cardId) => promoteRelatedCard(cardId)));
      setSelected([]);
      await reload();
    } catch (bulkError) {
      setError(bulkError instanceof Error ? bulkError.message : "Could not make the selected entries flashcards.");
  ...
  async function openConnections(row: RelatedCatalogueRow) {
    const sourcePack = packMap.get(row.pack_id);
    const [card] = await listCardsByIds([row.target_card_id]).catch(() => []);
    const targetPack = card ? packMap.get(card.pack_id) : null;
    const nextPack = targetPack ?? sourcePack;
    if (!nextPack || !card || treeBusyId) {
      if (!card) setError("Could not load this entry.");
      return;
    }
    setTreeBusyId(row.relation_id); setError("");
    try {
      setConnections({ pack: nextPack, card });
  ...
  return <>
    <section className="related-catalogue-page vocabulary-page">
      ...
      <header className="vocabulary-heading">
        <div><p className="eyebrow">{collection ? "RAW" : "CONNECTIONS"}</p><h1>{collection ? "Raw" : "Connections"}</h1><p>{collection ? `Uncatalogued knowledge connected to ${collection.title}.` : "Connected knowledge across Heuresis."}</p></div>
      ...
      {editing ? <div className="related-edit-word vocabulary-inline-editor vocabulary-edit-editor">
        ...
        <input autoFocus value={editing.term} onChange={(event) => setEditing((current) => current ? { ...current, term: event.target.value } : current)} placeholder="Entry / concept" />
        <input value={editing.reading} onChange={(event) => setEditing((current) => current ? { ...current, reading: event.target.value } : current)} placeholder="Context / reading" />
        <input value={editing.meaning} onChange={(event) => setEditing((current) => current ? { ...current, meaning: event.target.value } : current)} placeholder="Meaning / note" onKeyDown={(event) => { if (event.key === "Enter" && editing.term.trim() && !editBusy) void saveEdit(); }} />
      ...
      <div className="vocabulary-toolbar">
        <label className="vocabulary-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search entry, meaning or source" /></label>
        <span className="vocabulary-result-count">{uniqueEntries.toLocaleString()} {uniqueEntries === 1 ? "entry" : "entries"}</span>
        ...
        {!collection && availablePacks.length ? <details className="vocabulary-filter-menu vocabulary-scope-menu">
          ...
          <div className="vocabulary-filter-popover vocabulary-scope-popover">
            <button className={!scopeFilterCount ? "selected" : ""} onClick={(event) => { event.preventDefault(); setCollectionFilters([]); setPackFilters([]); }}><span>All connections</span><small>{uniqueEntryCount(collectionRows)}</small></button>
            {availableCollections.length ? <section><p>Collections</p>{availableCollections.map((item) => {
              const count = uniqueEntryCount(collectionRows.filter((row) => packMap.get(row.pack_id)?.collection_id === item.id));
        ...
        <details className="vocabulary-filter-menu">
          ...
          <div className="vocabulary-filter-popover">
            <button className={statusFilters.includes("raw") ? "selected" : ""} onClick={(event) => { event.preventDefault(); setStatusFilters((current) => toggleValue(current, "raw")); }}><span>Raw only</span><small>{uniqueEntryCount(collectionRows.filter((row) => statusOf(row) === "raw"))}</small></button>
            <button className={statusFilters.includes("flashcard") ? "selected" : ""} onClick={(event) => { event.preventDefault(); setStatusFilters((current) => toggleValue(current, "flashcard")); }}><span>Flashcard</span><small>{uniqueEntryCount(collectionRows.filter((row) => statusOf(row) === "flashcard"))}</small></button>
          </div>
        </details>

        {availableRelationTypes.length ? <details className="vocabulary-filter-menu">
          ...
          <div className="vocabulary-filter-popover">
            {availableRelationTypes.map((value) => <button key={value} className={relationFilters.includes(value) ? "selected" : ""} onClick={(event) => { event.preventDefault(); setRelationFilters((current) => toggleValue(current, value)); }}><span>{relationLabel(value)}</span><small>{uniqueEntryCount(collectionRows.filter((row) => row.relation_type === value))}</small></button>)}
          </div>
        </details> : null}
      ...
      {selected.length ? <div className="vocabulary-selection-bar">
        ...
        <details className="vocabulary-selection-menu">
          ...
          <div>
            {selectedRawCardIds.length ? <button disabled={bulkBusy} onClick={(event) => { event.preventDefault(); void promoteSelected(); }}><Plus size={14} /><span>Make flashcards</span><small>{selectedRawCardIds.length}</small></button> : null}
      ...
      {loading ? <div className="content-state compact">Loading connections…</div> : error ? <div className="content-state error-state compact">{error}</div> : <div className="vocabulary-table">
        <div className="vocabulary-table-head">
          <button onClick={() => toggleSort("word")}>Entry {sortKey === "word" ? (sortDirection === "asc" ? "↑" : "↓") : ""}</button>
        ...
        {shown.map((row) => {
          ...
          >
            ...
            <span className={`vocabulary-status ${statusOf(row)}`}>{statusOf(row) === "flashcard" ? "Flashcard" : "Raw"}</span>
        ...
        {!shown.length ? <div className="catalogue-empty">{collectionRows.length ? "No connections match these filters." : collection ? "No Raw entries here yet." : "No connections here yet."}</div> : null}
      </div>}
    </section>
