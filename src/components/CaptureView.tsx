...
import { createCard, type Collection, type PackWithType } from "../lib/heuresis";
...
export default function CaptureView({ collections, packs, initialPack, onBack }: Props) {
  ...
  useEffect(() => {
    if (!availablePacks.some((item) => item.id === packId)) setPackId(availablePacks[0]?.id ?? "");
  ...
  async function save(event: FormEvent) {
    ...
    setStatus("");
    try {
      await createCard(pack, values, note);
      setValues({});
      setNote("");
      setStatus("Saved to Heuresis");
    } catch (error) {
    ...
    <section className="capture-page">
      ...
      <div className="capture-layout">
        <aside className="capture-context">
          ...
          <h1>Add directly to Heuresis.</h1>
          <p>No intermediate inbox and no Anki export. Saving here creates a normal card in the selected pack.</p>
          <label>Collection
        ...
        <form className="capture-card" onSubmit={save}>
          <div className="capture-card-head">
            <div><p className="eyebrow">NEW CARD</p><h2>{pack?.title || "Choose a pack"}</h2></div>
            <Plus size={21} />
          </div>

          {!pack ? <div className="capture-empty">Create or select a pack before capturing a card.</div> : null}
          {pack && !fields.length ? <div className="capture-empty">This pack has no readable field schema yet.</div> : null}

          {fields.map((field, index) => (
            <label className="field-row" key={field.key}>
              <span>{field.label}{field.required ? <b> *</b> : null}</span>
              {field.role === "example" || field.role === "extra" ? (
                <textarea
                  rows={3}
                  autoFocus={index === 0}
          ...
          <div className="capture-actions">
            <span className={status.toLowerCase().includes("saved") ? "save-status success" : "save-status"}>{status}</span>
            <button className="primary-button" disabled={!pack || !fields.length || saving} type="submit"><Check size={16} />{saving ? "Saving…" : "Save card"}</button>
          </div>
        </form>
      </div>
    </section>
