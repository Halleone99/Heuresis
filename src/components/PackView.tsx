...
function CardEditor({ pack, card, tags, onClose, onSaved, onDeleted }: {
  pack: PackWithType;
  card: CardWithStats;
  tags: HeuresisTag[];
  onClose: () => void;
  ...
  const [values, setValues] = useState<Record<string, string>>(() => Object.fromEntries(
    (pack.cardType?.field_schema ?? []).map((field) => [field.key, fieldText(card.data, field.key)]),
  ));
  ...
  return (
    <section className="pack-page">
      ...
      <div className="pack-toolbar">
        <label className="pack-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this topic" /></label>
        <div className="filter-strip"><Filter size={14} />{([['all','All'],['new','Never encountered'],['favourite','Favourites'],['interesting','Interest 4–5'],['again','Often Again']] as Array<[FilterKey,string]>).map(([key,label]) => <button key={key} className={filter === key ? "selected" : ""} onClick={() => setFilter(key)}>{label}</button>)}</div>
      </div>
      ...
      {!loading && !error ? <div className="card-table">
        <div className="card-table-head"><span>{shown.length} shown</span><span>{cards.length}{pack.card_count > cards.length ? ` / ${pack.card_count}` : ""} loaded</span></div>
        {shown.map((card) => <button className="card-data-row" key={card.id} onClick={() => setEditing(card)}>
          <span className="card-term"><strong>{fieldText(card.data, term?.key) || "Untitled"}</strong>{reading ? <em>{fieldText(card.data, reading.key)}</em> : null}</span>
          <span className="card-meaning"><span>{fieldText(card.data, meaning?.key)}</span>{card.tags.length ? <span className="row-tags">{card.tags.slice(0,4).map((tag) => <i key={tag.id} className={tag.is_badge ? "badge" : ""}>{tag.name}</i>)}{card.tags.length > 4 ? <i>+{card.tags.length - 4}</i> : null}</span> : null}</span>
          <span className="card-tally">{card.interest_rank ? <b>{card.interest_rank}</b> : null}{card.stats.encounter_count ? `${card.stats.encounter_count}×` : "—"}{card.favourite ? <Star size={12} fill="currentColor" /> : null}{card.note ? <span title="Has note">✎</span> : null}</span>
        </button>)}
