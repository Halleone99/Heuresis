import { ArrowLeft, BookOpen, Brain, Filter, Link2, Plus, Search, Star } from "lucide-react";
...
type Props = { pack: PackWithType; collection: Collection | null; onBack: () => void; onCapture: () => void; onChanged: () => void; };

function CardEditor({ pack, card, tags, onClose, onSaved, onDeleted, onChanged }: { pack: PackWithType; card: CardWithStats; tags: HeuresisTag[]; onClose: () => void; onSaved: () => void; onDeleted: () => void; onChanged: () => void; }) {
  ...
  async function save() { setSaving(true); setMessage(""); try { await updateCard(pack, card.id, values, { note, favourite, interest_rank: interest, tagIds }); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save card."); } finally { setSaving(false); } }
  async function remove() { if (!window.confirm("Delete this card permanently?")) return; setSaving(true); try { await deleteCard(card.id); onDeleted(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not delete card."); setSaving(false); } }

  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className="card-editor-modal" role="dialog" aria-modal="true">
...
export default function PackView({ pack, collection, onBack, onCapture, onChanged }: Props) {
  ...
  async function reload() { setLoading(true); setError(""); try { const [nextCards, nextTags] = await Promise.all([listCards(pack.id), listTags()]); setCards(nextCards); setTags(nextTags); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : "Could not load cards."); } finally { setLoading(false); } }
  ...
  const shown = useMemo(() => cards.filter((card) => {
    ...
    return Object.values(card.data).flatMap((value) => Array.isArray(value) ? value : [value]).filter(Boolean).join(" ").toLowerCase().includes(needle) || card.tags.some((tag) => tag.name.toLowerCase().includes(needle));
  }), [cards, filter, query]);
  const explored = pack.card_count ? Math.round((pack.encountered_cards / pack.card_count) * 100) : 0;

  async function afterEdit() { setEditing(null); await reload(); onChanged(); }
  async function afterStudy() { await reload(); onChanged(); }
  async function afterRelatedChange() { await reload(); onChanged(); }

  if (relatedOpen) return <RelatedView pack={pack} onBack={() => setRelatedOpen(false)} onChanged={() => void afterRelatedChange()} />;

  return <section className="pack-page">
    <button className="text-button back-button" onClick={onBack}><ArrowLeft size={15} /> {collection?.title || "Library"}</button>
    <header className="pack-page-head"><div><p className="eyebrow">TOPIC</p><h1>{pack.title}</h1>{pack.description ? <p>{pack.description}</p> : null}<span className="pack-record">{pack.card_count} cards · {pack.encountered_cards} encountered · {explored}% explored · opened {pack.open_count}×</span></div><div className="editor-meta-row"><button className="text-button" onClick={() => setRelatedOpen(true)}><Link2 size={16} /> Related</button><button className="text-button" disabled={!cards.length} onClick={() => setStudyOpen(true)}><Brain size={16} /> Flashcards</button><button className="primary-button" onClick={onCapture}><Plus size={16} /> Add card</button></div></header>
    ...
    {editing ? <CardEditor pack={pack} card={editing} tags={tags} onClose={() => setEditing(null)} onSaved={() => void afterEdit()} onDeleted={() => void afterEdit()} onChanged={() => void afterRelatedChange()} /> : null}
