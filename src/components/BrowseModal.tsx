...
import { finishSession, recordEncounter, startSession } from "../lib/advanced";
...
export default function BrowseModal({ pack, cards, onClose, onComplete }: Props) {
  ...
  useEffect(() => {
    let active = true;
    void startSession(pack.id, "browse").then((id) => { if (active) setSessionId(id); }).catch(() => undefined);
    return () => { active = false; };
  }, [pack.id]);

  useEffect(() => {
    if (!sessionId || !card || seen.current.has(card.id)) return;
    seen.current.add(card.id);
    void recordEncounter(card.id, pack.id, sessionId).catch(() => undefined);
  }, [card, pack.id, sessionId]);

  async function close() {
    if (sessionId) await finishSession(sessionId).catch(() => undefined);
    onComplete(); onClose();
  }
  ...
  if (!card) return null;
  return <div className="immersive-layer browse-layer"><header className="immersive-bar"><span><b>{pack.title}</b> · Browse · {index + 1} / {cards.length}</span><button onClick={() => void close()}><X size={16} /> Close</button></header><main className="browse-stage"><article className="browse-card"><div className="browse-main"><p className="eyebrow">CARD {index + 1}</p><h1>{fieldText(card.data, term?.key) || "Untitled"}</h1>{reading ? <h2>{fieldText(card.data, reading.key)}</h2> : null}<div className="browse-meaning">{fieldText(card.data, meaning?.key)}</div></div>{pack.cardType?.field_schema.filter((field) => !primary.has(field.key) && fieldText(card.data, field.key)).length ? <div className="browse-details">{pack.cardType.field_schema.filter((field) => !primary.has(field.key) && fieldText(card.data, field.key)).map((field) => <div key={field.key}><span>{field.label}</span><p>{fieldText(card.data, field.key)}</p></div>)}</div> : null}{card.tags.length ? <div className="browse-tags">{card.tags.map((tag) => <span className={tag.is_badge ? "badge" : ""} key={tag.id}>{tag.name}</span>)}</div> : null}{card.note ? <aside>{card.note}</aside> : null}</article></main><footer className="immersive-controls"><button className="secondary-button" disabled={index === 0} onClick={() => setIndex((value) => Math.max(0, value - 1))}><ArrowLeft size={15} /> Previous</button><span>{seen.current.size} encountered this browse</span><button className="primary-button" onClick={() => { if (index >= cards.length - 1) void close(); else setIndex((value) => value + 1); }}>{index >= cards.length - 1 ? "Finish" : <>Next <ArrowRight size={15} /></>}</button></footer></div>;
}
